...
class _BaseDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        image_id, image, label, cls_label = self._load_data(index)
        if self.augment:
        ...
        return image_id, image.astype(np.float32), label.astype(np.int64), cls_label
    ...
    def __repr__(self):
        ...
        fmt_str += "    Split: {}\n".format(self.split)
        fmt_str += "    Root: {}\n".format(self.root)
        fmt_str += "    GT path: {}".format(self.gt_path)
        return fmt_str
