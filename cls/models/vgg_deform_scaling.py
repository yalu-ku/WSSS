...
class OffsetScaler_Learnable(nn.Module):
    def __init__(self, in_ch=512, out_ch=18):
        ...
        self.out_ch = out_ch 
        self.scale = nn.Sequential(
            nn.Conv2d(512, 18, kernel_size=3),
        )
    ...
    def forward(self, x):
        b, c, _, _ = x.size()
        x = self.scale(x)
        threshold = self.global_avg_pool(x).view(b, c)
        threshold = self.fc(threshold)
        return threshold, x 
