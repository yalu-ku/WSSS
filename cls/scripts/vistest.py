...
def validate(current_epoch):
    ...
    imglist = []
    for i in range(0, 5):
        result_vis = output_visualize(img[i], cam[i], label[i], gt_map[i], pred_map[i])
        results.append(result_vis[1])
        imglist.append(result_vis[0])
    
    results2 = []
    for i in range(0, 5):
        image = cv2.addWeighted(imglist[i], 0.5, results[i], 0.5, 0)
        # vis = np.transpose(results[i], (1, 2, 0)) * 255
        # vis = vis.astype(np.uint8)
        vis = np.transpose(image, (1, 2, 0)) * 255
        vis = vis.astype(np.uint8)
        image = Image.fromarray(vis).convert('RGB')
        results2.append(image)

    titles = [f"CAM_{i}" for i in range(1, 6)]
    wandb.log({ 
