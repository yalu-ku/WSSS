...
parser.add_argument("--img_dir", type=str, default="/HDD/dataset/VOC2012/")
parser.add_argument("--test_list", type=str, default='/home/junehyoung/code/wsss_baseline/voc2012_list/train_aug_cls.txt')
parser.add_argument("--checkpoint", type=str, default='/home/junehyoung/code/wsss_baseline/cls/checkpoints/baseline_on_test2/best.pth')
...
for idx, dat in enumerate(tqdm(data_loader)):
    ...
    for s in [256, 320, 384]:
        ...
        _, cam = model(_img, label, size=(H, W)) # (1, 20, H, W)
        # cam = model(_img, label, size=(H, W))

        """ obtain CAMs """
        cam = cam[0].cpu().detach().numpy() # (20, H, W)
