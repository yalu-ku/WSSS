...
class RepLKNet(nn.Module):
    ...
    def __init__(self, large_kernel_sizes, layers, channels, drop_path_rate, small_kernel,
        ...
        if num_classes is not None:
            ...
            self.gap = nn.Conv2d(2048,20,kernel_size=1) ## XL
...
def replk(pretrained=True, delta=0.55):
    ...
    model = create_RepLKNetXL(num_classes=2048, use_checkpoint=False) ## B->L, clsses 1000 -> 21~
    if pretrained:
        # model.load_state_dict(torch.load('/root/WSSS/metadata/RepLKNet-31B_ImageNet-1K_384.pth'), strict=False)
        model.load_state_dict(torch.load('/root/WSSS/metadata/RepLKNet-XL_MegData73M_pretrain.pth'), strict=False)
        # model.cuda()
    return model
